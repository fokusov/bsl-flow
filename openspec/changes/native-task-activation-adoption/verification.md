...
Решением владельца от 2026-09-16 изменение `native-cross-platform-cli` откачено: Go-контроллер удалён из репозитория вместе с `cli/`. ACCEPT выше сохраняется как история приёмки; в силе остаются PS-часть результатов: PS-наборы, реальные PS-валидаторы и controller-контракты на стороне PowerShell. Go-специфичные пункты (lane cli, `bsl-flow.exe`, Go-тесты) — исторические записи. Записи repository-store v2 больше недоступны; см. дополнение в `repository-task-registry`. Запись отката — `native-cross-platform-cli/rollback.md`.
