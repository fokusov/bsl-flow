...
`scripts/Invoke-BSLFlowTask.ps1` accepts `-Action`, `-ProjectPath`, optional `-TaskId`, `-InputFile`, `-AttemptId`, and `-CodexPath` (native executable). Paths containing spaces must be passed as PowerShell arguments, never concatenated shell commands. The supported Windows runtime is PowerShell 7 at the standard machine path `C:\Program Files\PowerShell\7\pwsh.exe`; Windows PowerShell 5.1 fallback is not supported.
