...
3. Call `-Action Run -ProjectPath <root> -TaskId <uuid>` once. The controller owns stage sequencing, isolated worktree, reviews, verification and acceptance. Runtime gates requiring an unconfirmed adapter or target remain `BLOCKED`.
4. Read the JSON envelope. Show the specific pending question/blocker, or hand off the accepted worktree and receipt. Acceptance does not merge, publish, deploy, or authorize database changes.
...
The first adapter supports the verified Windows native Codex CLI version and explicit source-only permissions described in the contract. It blocks source-controlled execution configuration. Do not silently relax isolation or substitute an unavailable reviewer. The existing OpenCode spec reviewer keeps its configured model; the task's Codex reviewer applies to code review.

This release does not execute 1C runtime mutations through managed mode. Keep the existing temporary Unica runtime restriction. A prepared scenario or source-only pilot is not 1C runtime acceptance.
