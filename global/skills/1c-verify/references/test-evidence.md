...
The request declares the runner, `operation: test`, exact FILE target, source-set/extension identities, exact test selection, relevant versions, durable `reportPath`, required route `capabilities`, and an explicit JSON boolean `noBuild`; `noLoad` is checked when declared. The observed file is a saved preview/receipt from the selected public route and must contain an `effective` object with target, sources, selection, versions, report path and capabilities; copied request fields at the envelope root are not treated as effective state. The result compares declared and observed values and reports `PASS` only when the route, target, sources, selection, versions, schema and no-build evidence are present. A test-module filter is not no-build evidence. When `noBuild` or `noLoad` is effective, every matching build/load step must exist and be explicitly skipped for that reason; an executed, mixed, missing or unknown required step, or a declared/effective flag conflict, blocks preflight. Missing observed source/version/profile evidence is a blocker, not an inference. `launch_performed` is always false.
...
`expected.json` must contain a non-empty `selection` (or `tests`), positive `total`, exact `target`, `sources` and `versions` (the selection count is used when total is omitted). Receipt counts and JUnit counts are checked against the expected total and each other; failed, errored, skipped, zero, stale, malformed, missing or mismatched evidence cannot become `PASS`. The original JUnit is mandatory for the existing and legacy contract: an omitted `-JUnitPath` and a supplied missing path both record `junit_status: missing_or_deleted` and remain `BLOCKED`. A parsed runner receipt is not the original JUnit, and there is no receipt-only profile in this contract.

`-SourceManifestPath` is also mandatory for `PASS`. It points to a fresh JSON manifest with `schema_version: 1`, `kind: bsl-flow.source-manifest`, `complete: true`, exact ordered `sources`, a non-empty `files` array, and an explicit `generated_exclusions` array. `coverage.tracked`, `.untracked`, `.deleted` and `.generated_exclusions` must all be JSON `true`; the completeness claim is rejected without this coverage declaration and actual file entries. Every file entry has a unique non-empty `path`, `state` equal to `tracked`, `untracked` or `deleted`, and a 64-character hexadecimal `sha256` (for a deletion, the hash identifies the removed source bytes). Missing, malformed, incomplete, source-mismatched or stale manifests remain `BLOCKED`; a version string alone does not prove source byte identity.
