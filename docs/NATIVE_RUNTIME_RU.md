...
Контракт принимает точную версию `8.3.*` либо `8.5.1+` в ветке 8.5. Поддержка 8.5 на уровне схемы и офлайн-проверок добавлена отдельно от приведённого выше пилота 8.3: загрузка, COM inventory, YAxUnit и восстановление на 8.5 ещё требуют реального запуска на выделенной локальной FILE-копии. Версия `8.4.*` и предварительная `8.5.0.*` отклоняются.
