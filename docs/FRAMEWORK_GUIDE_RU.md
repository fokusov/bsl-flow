...
Managed не выполняет merge, push, publish, deploy, глобальную установку или операцию с базой 1С. Acceptance receipt разрешает передать проверенную worktree основному агенту; внешнее действие требует своего разрешения.
...
Cancel запрещает новый dispatch и останавливает принадлежащее runner дерево процессов, но не откатывает уже применившееся внешнее действие. Для продолжения после Cancel нужен явный пользовательский `Update` с `kind: authorization` и `resume: true`, затем `Resume`.
