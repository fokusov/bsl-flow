...
On cancellation, verify the saved process identity before terminating its owned tree. The observed tree cancellation is not proof of rollback, safety of detached processes, or completion of a previously sent business mutation. Unknown business results require a control read. Earlier argument quoting, UTF-8 stdin, bounded unread stdin, cancellation and recovery tests passed in PowerShell 5.1 and 7; those results remain historical evidence. The supported runtime now requires PowerShell 7, uses native ArgumentList and explicit UTF-8 encoding, and launches child pwsh from the controller's PSHOME.
