...
Статус документа: решения первой source-only реализации для `0.7.0-dev.1`. Настоящий source-only host pilot прошёл автоматический маршрут до acceptance. Managed-адаптер 1С runtime ещё не подтверждён и отключён; также сохраняется временное ограничение Unica. Документ не утверждает завершение M8 или полную автономность. Текущие результаты приведены в [отчёте проверки](../VERIFICATION.md).
...
**Почему.** Подготовленный тест, локальный sentinel или успешная сборка не доказывает загрузку, UI-поведение либо бизнес-результат в конкретной базе. Временное ограничение Unica запрещает managed runtime mutations до отдельно подтверждённого восстановления и разрешения пользователя.
