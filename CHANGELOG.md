...
- Package building skips generated and ignored root directories before recursive enumeration, so inaccessible files under `work` do not prevent a release ZIP.
- Managed Codex workers now accept stable CLI versions from 0.153.0 onward only after the existing read/write sandbox capability probes succeed; profiled execution also requires its pinned provider and sandbox versions to match. Versions with changed command, event, or isolation behavior remain BLOCKED by the relevant gate. The sealed current-agent Council fallback retains its separately verified executable contract.
- The native FILE request contract admits 1C 8.5.1 and later 8.5 patch releases while retaining exact executable/COM binding, physical FILE identity, protected tests, original JUnit, and recovery gates. 8.5 runtime acceptance remains pending a real isolated-base pilot.
